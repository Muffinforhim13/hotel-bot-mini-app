#!/usr/bin/env python3
"""
Запуск Mini App API сервера для Hotel Bot (версия для Windows)
"""
...
def check_requirements():
    ...
    if missing_packages:
        logger.error(f"ОШИБКА: Отсутствуют необходимые пакеты: {', '.join(missing_packages)}")
...
def check_mini_app_files():
    ...
    if missing_files:
        logger.error(f"ОШИБКА: Отсутствуют файлы Mini App: {', '.join(missing_files)}")
        return False
